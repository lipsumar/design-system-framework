...
var API = {
            ...
            configValidator(CONFIG, function(err, configStatus){
                if(configStatus.canStart){
                    ...
                    if(CONFIG.plugins){
                        ...
                        async.series(CONFIG.plugins.map(function(pluginName){
                            return require(util.path.absolute(pluginName)).bind(null, API);///@TODO modify API for plugins (special log, ...)
                        }), callback);
    ...
    registerInspectorPlugin: function(pluginId, options){
        inspectorPluginsById[pluginId] = _.clone(options);
        inspectorPluginsById[pluginId].id = pluginId;
    },
