...
var CONFIG = _.extend({}, defaultOptions, userOptions);
 ...
 */
function addComponentPathCandidate(path, componentId){
		...
		if(CONFIG['external-components']){
			_.each(CONFIG['external-components'], function(external, componentId){
				...
				if(external['components-path']){
					fetchComponents(external['components-path']);
				}else if(external['component-path']){
					addComponentPathCandidate(external['component-path'], componentId);
				}
