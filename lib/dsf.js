...
var API = {
	...
	init: function(configFile){
		configFile = configFile || 'options.json';

		try{
			userOptions = require(path.join(process.cwd(), configFile));
		}catch(err){
			userOptions = {};
		}

		CONFIG = _.merge({}, defaultOptions, userOptions);

		// disable API.init
		API.init = util.func.noop;
	},

	start: function(){
		// disable API.start
		API.start = util.func.noop;

		// cleanup previous files
		API.util.path.remove('public/_built', {glob:false}, function(){
